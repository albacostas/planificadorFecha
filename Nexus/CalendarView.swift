...
struct CalendarView: View {
    ...
    func dayCell(for date: Date) -> some View {
        ...
        let eventsOnDay = eventData.eventsOn(date: date)
        ...
        Button {
            ...
            VStack(spacing: 4) {
                ...
                // 2. CORREGIDO: Usar defaultEventColor (RGBAColor) en la coalescencia.
                    .foregroundColor(isCurrentMonth ? (isToday ? .white : (isSelected ? Color(eventData.allEvents.first?.color ?? defaultEventColor) : .primary)) : .secondary.opacity(0.5))
            ...
            .background {
                if isToday {
                    ...
                    Circle().stroke(Color(eventData.allEvents.first?.color ?? defaultEventColor), lineWidth: 2)
                }
            }
        }
